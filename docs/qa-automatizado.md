...
| `qa_all.ps1` | — | **Runner**: corre los ocho y resume (esperado: `TODOS LOS ARNESES OK`, **154 checks**). OJO: su acumulador se llama `$arnesesFallidos` — con `pwsh -File` el runner corre en scope global y un `$fail` propio era pisado por el `$global:fail = 0` de cada arnés (bug real corregido el 2026-07-16: una suite con fallos reportaba TODOS OK). |
