...
foreach ($resbienesAgrupados as $respoas) {
    ...
    // La combinación de celdas se realiza después de insertar los encabezados de fuentes
    ...
    if ($fuentesPrograma[$respoas[0]->id_programa]) {
        ...
        if ($fte) {
            ...
            //Combinamos las celdas para el título hasta donde se hayan insertado todos los datos
            // Combinar celdas para la cabecera
            $sheet->mergeCells("A" . ($cntrows + 1) . ":I" . ($cntrows + 1)); // Combina desde A2 hasta I2
            $sheet->setCellValue("A" . ($cntrows + 1), 'PRESUPUESTO - ' . date('Y') . ' - PROGRAMA ' . strtoupper($respoas[0]->programa));
            ...
            $ultimaColFuente = $cols[$contespecial - 1];
            $sheet->mergeCells("K" . ($cntrows + 2) . ":{$ultimaColFuente}" . ($cntrows + 2));
            // Dar estilos de negrita y tamaño
            $sheet->getStyle("K" . ($cntrows + 2) . ":{$ultimaColFuente}" . ($cntrows + 2))->getFont()->setBold(true)->setSize(12);
            $sheet->getStyle("K" . ($cntrows + 2) . ":{$ultimaColFuente}" . ($cntrows + 2))->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
            $sheet->getStyle("K" . ($cntrows + 2) . ":{$ultimaColFuente}" . ($cntrows + 2))->getAlignment()->setWrapText(true);
        }
    }
